...
export function SettingsSection(props: SettingsSectionProps) {
  ...
  return <div className="space-y-6"><AdminPageHeader title="Paramètres" subtitle="Gérez l’identité de marque, le footer et les métadonnées publiques." />
    ...
    {activeTab === 'branding' ? <div className="grid gap-4 md:grid-cols-2"><BrandAsset field="logo" title="Logo principal" optional={false} /><BrandAsset field="logoDark" title="Logo sombre" /><BrandAsset field="favicon" title="Favicon" /><BrandAsset field="defaultSocialImage" title="Image sociale par défaut" /></div> : null}
    ...
    {activeTab === 'social' ? <AdminPanel title="Liens sociaux du footer"><div className="space-y-3">{settingsValues.footer.socialLinks.map((link, index) => <div key={`${link.platform}-${index}`} className="grid gap-2 rounded-[14px] border border-[#e1ebef] bg-[#fbfdfe] p-3 lg:grid-cols-[150px_160px_1fr_auto_auto]"><select value={link.platform} onChange={(event) => updateSocial(index, { platform: event.target.value, label: socialLabel(event.target.value) })} className="rounded-[9px] border border-[#d8e4e8] px-2 py-2">{SOCIAL_PLATFORMS.map((platform) => <option key={platform} value={platform}>{socialLabel(platform)}</option>)}</select><input aria-label="Libellé" value={link.label} onChange={(event) => updateSocial(index, { label: event.target.value })} className="rounded-[9px] border border-[#d8e4e8] px-2 py-2" /><div><input aria-label="URL" value={link.url} onChange={(event) => updateSocial(index, { url: event.target.value })} className={`w-full rounded-[9px] border px-2 py-2 ${link.url && !isValidSocialUrl(link.url) ? 'border-red-400' : 'border-[#d8e4e8]'}`} placeholder={link.platform === 'email' ? 'mailto:contact@example.com' : 'https://...'} />{link.url && !isValidSocialUrl(link.url) ? <p className="mt-1 text-[11px] text-red-600">URL invalide. Utilisez https:// ou mailto:.</p> : null}</div><label className="flex items-center gap-2 text-[12px]"><input type="checkbox" checked={link.enabled} onChange={(event) => updateSocial(index, { enabled: event.target.checked })} /> Actif</label><div className="flex gap-1"><button type="button" aria-label="Monter" onClick={() => moveSocial(index, -1)} className="rounded border p-2"><ArrowUp size={14} /></button><button type="button" aria-label="Descendre" onClick={() => moveSocial(index, 1)} className="rounded border p-2"><ArrowDown size={14} /></button><button type="button" aria-label="Supprimer" onClick={() => setSettingsValues((prev) => ({ ...prev, footer: { socialLinks: prev.footer.socialLinks.filter((_, itemIndex) => itemIndex !== index) } }))} className="rounded border border-red-200 p-2 text-red-600"><Trash2 size={14} /></button></div></div>)}<button type="button" onClick={() => setSettingsValues((prev) => ({ ...prev, footer: { socialLinks: [...prev.footer.socialLinks, { platform: 'website', label: 'Site web', url: 'https://', enabled: true }] } }))} className="inline-flex items-center gap-2 rounded-[10px] bg-[#00a590] px-4 py-2 text-[13px] font-semibold text-white"><Plus size={15} />Ajouter un lien</button></div></AdminPanel> : null}
