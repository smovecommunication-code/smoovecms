...
import { isValidSocialLinkUrl, normalizeSocialUrl, type CmsSettings, type ContentHealthSummary, type ConversionMetrics, type SettingsHistoryEntry, type SocialLink } from '../../../utils/contentApi';
...
const isValidSocialUrl = (value: string, platform = '') => isValidSocialLinkUrl(value, platform);
...
export function SettingsSection(props: SettingsSectionProps) {
  ...
  const updateSocial = (index: number, patch: Partial<SocialLink>) => setSettingsValues((prev) => ({ ...prev, footer: { socialLinks: prev.footer.socialLinks.map((link, itemIndex) => itemIndex === index ? { ...link, ...patch, url: patch.url !== undefined ? normalizeSocialUrl(patch.url, patch.platform || link.platform) : link.url } : link) } }));
  ...
  return <div className="space-y-6"><AdminPageHeader title="Paramètres" subtitle="Gérez l’identité de marque, le footer et les métadonnées publiques." />
    ...
    <AdminActionBar><div className="text-[12px] text-[#60737b]">{settingsHasUnsavedChanges ? 'Modifications non sauvegardées.' : 'Paramètres à jour.'}</div><AdminButton onClick={saveSettings} disabled={settingsSaving || settingsValues.footer.socialLinks.some((link) => !link.label.trim() || !link.platform.trim() || !isValidSocialUrl(link.url, link.platform))}>{settingsSaving ? 'Sauvegarde...' : 'Sauvegarder les paramètres'}</AdminButton></AdminActionBar>
  </div>;
}
