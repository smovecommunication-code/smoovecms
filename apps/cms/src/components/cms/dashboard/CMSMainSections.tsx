...
import { getPublicBlogArticleUrl } from '../../../utils/publicSiteUrl';
...
function MediaLibraryThumbnail({ file }: { file: MediaFile }) {
  const thumb = resolveMediaLibraryThumbnail(file);
  const [failedUrl, setFailedUrl] = useState('');

  if (thumb.kind === 'image' && thumb.src && failedUrl !== thumb.src) {
    return <img src={getCloudinaryVariant(thumb.src, 'thumbnail')} alt={file.alt || file.name} className="aspect-[4/3] w-full object-cover" loading="lazy" onError={() => setFailedUrl(thumb.src || '')} />;
  }

  if (failedUrl) {
    return <div className="flex h-[124px] items-center justify-center px-3 text-center text-[11px] text-amber-700">Fichier introuvable: {failedUrl}</div>;
  }

  return <div className="flex h-[124px] items-center justify-center text-[12px] text-[#6f7f85]">{thumb.kind === 'missing' ? 'Image non résolue' : file.type === 'video' ? 'Aperçu vidéo' : 'Fichier' }</div>;
}
...
interface MediaSectionProps {
  ...
  canDeleteContent: boolean;
  updateSelectedMedia: (patch: Partial<MediaFile>) => Promise<void>;
  replaceSelectedMediaFile: (event: ChangeEvent<HTMLInputElement>) => Promise<void>;
  deleteMedia: (file: MediaFile, references?: BackendMediaReference[]) => Promise<void>;
}
...
export function MediaSection({
  ...
  const selectedAssetPreview = selectedMedia
    ...
    : null;
  const [metadataDraft, setMetadataDraft] = useState({ alt: '', caption: '', tags: '', label: '' });
  ...
  useEffect(() => {
    setMetadataDraft({
      ...
      label: selectedMedia?.label || selectedMedia?.title || selectedMedia?.name || '',
    });
  }, [selectedMedia?.id, selectedMedia?.alt, selectedMedia?.caption, selectedMedia?.label, selectedMedia?.name, selectedMedia?.tags]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-[2fr_1fr]"><AdminPanel title="Ressources médias">{filteredMediaFiles.length === 0 ? <AdminEmptyState label="Aucun média correspondant. Ajoutez des ressources ou modifiez la recherche." /> : <div className="grid gap-3 md:grid-cols-2">{filteredMediaFiles.map((file) => (<div key={file.id} className={`rounded-[14px] border p-3 transition ${selectedMediaId === file.id ? 'border-[#00b3e8] bg-[#f0fbff] shadow-[0_0_0_1px_rgba(0,179,232,0.15)]' : 'border-[#e4edf1] bg-[#fcfeff] hover:border-[#d6e4ea]'}`}><button type="button" onClick={() => setSelectedMediaId(file.id)} className="block w-full text-left"><div className="mb-2 aspect-[4/3] overflow-hidden rounded-[10px] border border-[#e5edf1] bg-gradient-to-br from-[#eef8fb] to-[#dcecf2]"><MediaLibraryThumbnail file={file} /></div><div className="space-y-1"><p className="font-['Abhaya_Libre:Bold',sans-serif] text-[#273a41]">{file.label || file.name}</p><p className="font-['Abhaya_Libre:Regular',sans-serif] text-[13px] text-[#6f7f85]">{file.type} • {Math.round(file.size / 1024)} KB</p><p className="text-[12px] text-[#8a969b]">{file.alt || 'alt non renseigné'}</p></div></button><div className="mt-3 border-t border-[#e4edf1] pt-3"><AdminButton type="button" onClick={() => void deleteMedia(file)} disabled={!canDeleteContent} intent="danger" size="sm" title={canDeleteContent ? 'Supprimer ce média' : 'Réservé aux administrateurs'}><Trash2 size={14} /> Supprimer</AdminButton></div></div>))}</div>}</AdminPanel><AdminPanel title="Détails du média">{!selectedMedia ? <AdminEmptyState label="Sélectionnez une ressource pour inspecter et gouverner cet asset." /> : <div className="space-y-4 text-[14px] text-[#4b5a60]"><div className={ADMIN_SECTION_SUBCARD}><div className="mb-2 flex items-center justify-between gap-2"><p className="text-[13px] font-semibold text-[#273a41]">Aperçu asset</p>{selectedAssetPreview ? <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[11px] font-medium ${previewToneClass(selectedAssetPreview.statusTone)}`}>{selectedAssetPreview.statusLabel}</span> : null}</div><div className="overflow-hidden rounded-[10px] border border-[#e4edf1] bg-[#f5f9fa]">{selectedAssetPreview?.state === 'resolvable' ? <img src={getCloudinaryVariant(selectedAssetPreview.src, 'contain')} alt={selectedAssetPreview.alt} className="max-h-[360px] w-full object-contain" loading="lazy" /> : <div className="flex h-[180px] items-center justify-center px-4 text-center text-[12px] text-[#6f7f85]">Prévisualisation non résolue. Vérifiez la disponibilité du média.</div>}</div><p className="mt-2 text-[12px] text-[#5f7178]">{selectedAssetPreview?.sourceLabel} · {selectedAssetPreview?.reference || 'Aucune référence'}</p></div><div className={ADMIN_SECTION_SUBCARD}><p className="mb-2 text-[13px] font-semibold text-[#273a41]">Métadonnées éditables</p><div className="space-y-2"><input value={metadataDraft.label} onChange={(event) => setMetadataDraft((prev) => ({ ...prev, label: event.target.value }))} className={ADMIN_INPUT_CLASS} placeholder="Titre / libellé" disabled={!canEditContent} /><input value={metadataDraft.alt} onChange={(event) => setMetadataDraft((prev) => ({ ...prev, alt: event.target.value }))} className={ADMIN_INPUT_CLASS} placeholder="Texte alternatif" disabled={!canEditContent} /><textarea value={metadataDraft.caption} onChange={(event) => setMetadataDraft((prev) => ({ ...prev, caption: event.target.value }))} className={ADMIN_TEXTAREA_CLASS} placeholder="Légende" disabled={!canEditContent} /><input value={metadataDraft.tags} onChange={(event) => setMetadataDraft((prev) => ({ ...prev, tags: event.target.value }))} className={ADMIN_INPUT_CLASS} placeholder="Tags séparés par des virgules" disabled={!canEditContent} /><AdminActionCluster><AdminButton type="button" onClick={() => void updateSelectedMedia({ label: metadataDraft.label.trim() || selectedMedia.name, title: metadataDraft.label.trim() || selectedMedia.name, alt: metadataDraft.alt.trim(), caption: metadataDraft.caption.trim(), tags: metadataDraft.tags.split(',').map((tag) => tag.trim()).filter(Boolean) })} disabled={!canEditContent} intent="primary" size="sm"><Pencil size={14} /> Enregistrer</AdminButton><label className="inline-flex cursor-pointer items-center gap-2 rounded-[9px] border border-[#d9e5ea] bg-white px-3 py-1.5 text-[12px] text-[#3e525a] transition hover:border-[#00b3e8]"><Upload size={14} /> Remplacer le fichier<input type="file" className="hidden" onChange={(event) => { void replaceSelectedMediaFile(event); }} disabled={!canEditContent || isUploadingMedia} /></label></AdminActionCluster></div><div className="mt-3 space-y-1 border-t border-[#e7eef2] pt-3"><p><span className={ADMIN_FIELD_LABEL_CLASS}>ID stable:</span> {selectedMedia.id}</p><p><span className={ADMIN_FIELD_LABEL_CLASS}>Fichier:</span> {selectedMedia.filename || selectedMedia.originalName || selectedMedia.name}</p><p><span className={ADMIN_FIELD_LABEL_CLASS}>MIME:</span> {selectedMedia.mimeType || 'n/a'}</p><p><span className={ADMIN_FIELD_LABEL_CLASS}>Source:</span> {selectedMedia.source || 'local-storage'}</p><p><span className={ADMIN_FIELD_LABEL_CLASS}>Créé:</span> {selectedMedia.createdAt || selectedMedia.uploadedDate}</p><p><span className={ADMIN_FIELD_LABEL_CLASS}>Mis à jour:</span> {selectedMedia.updatedAt || selectedMedia.uploadedDate}</p><p><span className={ADMIN_FIELD_LABEL_CLASS}>Référence:</span> <code className="rounded bg-[#f5f9fa] px-2 py-1 text-[12px]">{toMediaReferenceValue(selectedMedia.id)}</code></p></div></div><div className={ADMIN_SECTION_SUBCARD}><p className="mb-2 text-[13px] font-semibold text-[#273a41]">Gouvernance • Où utilisé (source serveur)</p>{authoritativeReferencesLoading ? <p className="text-[12px] text-[#6f7f85]">Analyse des références actives…</p> : null}{authoritativeReferencesError ? <p className="text-[12px] text-amber-700">{authoritativeReferencesError}</p> : null}{!authoritativeReferencesLoading ? <p className="text-[12px] text-[#5e7077]">{selectedReferenceSummary.total === 0 ? 'Aucune référence active détectée par le graphe backend.' : `${selectedReferenceSummary.total} référence(s) active(s) détectée(s).`}</p> : null}{selectedReferenceSummary.byDomain.length > 0 ? <ul className="mt-2 space-y-1 text-[12px] text-[#4b5a60]">{selectedReferenceSummary.byDomain.map((domain) => (<li key={domain.domain}>• {domain.label}: {domain.count}</li>))}</ul> : null}{selectedReferenceSummary.sample.length > 0 ? <ul className="mt-2 list-disc space-y-1 pl-5 text-[12px] text-[#5b6a70]">{selectedReferenceSummary.sample.map((usage) => (<li key={usage}>{usage}</li>))}</ul> : null}{!authoritativeReferencesLoading && authoritativeReferences.length === 0 && localFallbackUsages.length > 0 ? <p className="mt-2 text-[12px] text-amber-700">Indice local non-bloquant: {localFallbackUsages.slice(0, 3).join(' | ')}</p> : null}</div><div className={ADMIN_SECTION_SUBCARD}><p className="mb-2 text-[13px] font-semibold text-[#273a41]">Complétude métadonnées</p>{selectedMetadataCompleteness ? <ul className="space-y-1 text-[12px]"><li>Alt: {selectedMetadataCompleteness.alt ? 'présent' : 'manquant'}</li><li>Caption: {selectedMetadataCompleteness.caption ? 'présente' : 'manquante'}</li><li>Tags: {selectedMetadataCompleteness.tags ? 'présents' : 'manquants'}</li></ul> : null}</div><div className="rounded-[10px] border border-rose-200 bg-rose-50 px-3 py-3 text-[12px] text-rose-800"><p className="mb-2 font-semibold">Danger zone</p><p>Suppression Cloudinary protégée: impossible si des références actives existent.</p><p className="mt-1">{selectedReferenceSummary.total > 0 ? 'État actuel: suppression bloquée (références actives).' : 'État actuel: suppression autorisée (aucune référence active).'}</p><AdminActionCluster danger><AdminButton type="button" onClick={() => void deleteMedia(selectedMedia, authoritativeReferences)} disabled={!canDeleteContent || authoritativeReferencesLoading} intent="danger" size="sm">Supprimer ce média</AdminButton></AdminActionCluster></div></div>}</AdminPanel></div>
    </div>
  );
}
...
export function PageContentSection({
  ...
  const documentMediaOptions = useMemo(() => mediaFiles.filter((file) => file.type === 'document' || file.type === 'file'), [mediaFiles]);
                        ...
                        return (
                          <div className="mb-3 rounded-[10px] border border-[#e4edf1] bg-[#f8fbfd] p-2.5">
                            ...
                            {preview.state === 'resolvable' ? (
                              <img src={getCloudinaryVariant(preview.src, 'contain')} alt={preview.alt} className="max-h-[320px] w-full rounded-[8px] border border-[#e4edf1] bg-[#f5f9fa] object-contain" loading="lazy" />
                            ) : (
