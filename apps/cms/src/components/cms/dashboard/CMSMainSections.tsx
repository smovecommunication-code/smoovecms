...
function MediaLibraryThumbnail({ file }: { file: MediaFile }) {
  ...
  if (thumb.kind === 'image' && thumb.src && failedUrl !== thumb.src) {
    return <img src={thumb.src} alt={file.alt || file.name} className="h-[124px] w-full object-cover" loading="lazy" onError={() => setFailedUrl(thumb.src || '')} />;
  }
