...
const normalizeService = (service: Partial<Service> & { id: string }): Service => {
  ...
  return {
    ...
    summary: requiredTrimmed(service.summary) || undefined,
    detailImage: requiredTrimmed(service.detailImage) || undefined,
    detailCta: service.detailCta ? {
      title: requiredTrimmed(service.detailCta.title) || undefined,
      text: requiredTrimmed(service.detailCta.text) || undefined,
      buttonLabel: requiredTrimmed(service.detailCta.buttonLabel) || undefined,
      buttonUrl: requiredTrimmed(service.detailCta.buttonUrl) || undefined,
    } : undefined,
    icon: requiredTrimmed(service.icon) || 'palette',
    iconLikeAsset: requiredTrimmed(service.iconLikeAsset) || undefined,
    representativeImage: requiredTrimmed(service.representativeImage) || requiredTrimmed(service.visualMedia) || requiredTrimmed(service.iconLikeAsset) || undefined,
    illustrationCards: Array.isArray(service.illustrationCards) ? service.illustrationCards.filter((card) => card?.title && card?.image) : [],
